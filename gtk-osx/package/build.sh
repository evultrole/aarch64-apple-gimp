...
sudo cp "/usr/lib/charset.alias" "${HOME}/gtk/inst/lib/"
...
then
  ...
  find  ${PACKAGE_DIR}/GIMP-2.10.app/Contents/Resources/lib/ -type f -perm +111 \
     ...
     | xargs /usr/bin/codesign -s "${codesign_subject}" \
         --options runtime \
         --entitlements ${HOME}/Skunkworks/gtk-osx/package/gimp-hardening.entitlements
  find  ${PACKAGE_DIR}/GIMP-2.10.app/Contents/MacOS -type f -perm +111 \
     | xargs file \
     |awk -F ':' '{print $1}' \
     | xargs /usr/bin/codesign -s "${codesign_subject}" \
...
rm -f /tmp/tmp.dmg
rm -f "/tmp/artifacts/gimp-${GIMP_VERSION}-arm64.dmg"
