...
InsertMenuItemTextWithCFString (carbon_menu, cfstr, index - 1,
                                        attributes, 0);
...
SetMenuItemHierarchicalMenu (carbon_item->menu,
                                         carbon_item->index, NULL);

CheckMenuItem (carbon_item->menu, carbon_item->index, active);

ChangeMenuAttributes (carbon_item->submenu,
                                set_attrs, clear_attrs);

GetMenuItemProperty (carbon_item->menu, carbon_item->index,
                             GTK_QUARTZ_MENU_CREATOR,
                             GTK_QUARTZ_ITEM_WIDGET,
                             sizeof (checkWidget), 0, &checkWidget);

DeleteMenuItem (menu_item->menu, menu_item->index);
